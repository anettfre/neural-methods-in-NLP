...
class LMBert(SLM):
    ...
    def __init__(self, cuda_device, bert_model, max_batch_size=20):
        ...
        #import zipfile
        #with zipfile.ZipFile(bert_model,"r"):
        #myfile=zipfile.ZipFile(bert_model)
        #with zipfile.ZipFile("/cluster/shared/nlpl/data/vectors/latest/194.zip", "r") as archive:
        #    bert_model = archive.extract("model.hdf5")        
        #bert_model= myfile.extract('model.bin')
        with torch.no_grad():
            #model = transformers.TFBertForMaskedLM.from_pretrained(bert_model) #added            
            model = transformers.BertForMaskedLM.from_pretrained(bert_model)
            ...
            self._lemmas_cache = {}
            #self._spacy = nlp
            for spacyed in self.tokenizer.vocab.keys():
                lemma = spacyed[0].lower()#lemma_ if spacyed[0].lemma_ != '-PRON-' else spacyed[0].lower_
                self._lemmas_cache[spacyed[0].lower()] = lemma
                ...
                self.original_vocab.append(spacyed[0].lower())
    ...
    def predict_sent_substitute_representatives(self, inst_id_to_sentence: Dict[str, Tuple[List[str], int]],
        ...
        with torch.no_grad():
            ...
            for batch in get_batches(sorted_by_len,
                ...
                #logits_all_tokens.shape[2]
                logits_target_tokens = torch.zeros((len(batch_sents), 1024)).to(self.device)
                for i in range(0, len(batch_sents)):
                    logits_target_tokens[i, :] = logits_all_tokens.logits[i, batch_sents[i][1], :]
