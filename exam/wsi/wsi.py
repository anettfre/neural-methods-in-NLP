...
class WordSenseInductor:
    ...
    def _perform_wsi_on_ds_gen(self, ds_name, gen, wsisettings: WSISettings, eval_proc, print_progress=False):
        ...
        import json
        with open("data/results.key", "w", encoding="utf-8") as f:
             f.write(json.dumps(inst_id_to_sense))
    ...
    def run(self, wsisettings: WSISettings,
        ...
        scores2013,corr = self._perform_wsi_on_ds_gen(
            'SemEval2013',
            generate_sem_eval_2013_no_tokenization('provided/SemEval-2013-Task-13-test-data'),
            wsisettings,
            lambda inst2sense, outkey:
            evaluate_labeling_2013('provided/SemEval-2013-Task-13-test-data', inst2sense, outkey),
            print_progress=print_progress)
        from sklearn import metrics
        true_labels = generate_sem_eval_2013_no_tokenization('provided/SemEval-2013-Task-13-test-data')
        ari = metrics.adjusted_rand_score(true_labels, scores2013)
        logging.info(f'Adjusted rand score {scores2013}')
        ...
        return None, scores2013['all']#scores2010['all'], scores2013['all']
